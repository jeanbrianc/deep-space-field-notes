...
test("gallery ships the complete capture collection and interactions", async () => {
  ...
  assert.match(page, /Faint light, gathered patiently/);
  assert.match(page, /Cross the sky one field note at a time/);
  assert.match(page, /Skip introduction/);
  assert.match(page, /href="#field-notes"/);
  assert.match(page, /Enter the observatory/);
  assert.match(page, /data-entry-step/);
